...
/**
 * The cmd processes the INewCmdReqMsg
...
public class NewReqCmd extends AOurDataPacketAlgoCmd<INewCmdReqMsg,IChatServer> {
	...
	public Void apply(Class<?> index, OurDataPacket<INewCmdReqMsg,IChatServer> host, Object... params) {
		AOurDataPacketAlgoCmd<?,IChatServer> newCmd = ((IServer2ModelAdapter) params[0]).getCmd(host.getData().getReqClassIdx());
		IAddCmdMsg msg = new AddCmdMsg(newCmd, host.getData().getReqClassIdx(), host.getData().getUUID());
		OurDataPacket<IAddCmdMsg,IChatServer> m=new OurDataPacket<IAddCmdMsg,IChatServer>(IAddCmdMsg.class,msg,((IServer2ModelAdapter) params[0]).getStub());
		try {
